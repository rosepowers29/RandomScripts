...
n_avg=[]
for t in ts:
    rho_g1, rho_g2 = update_pop(rho_g1, rho_g2, dt, -1.0)
    rhos.append((rho_g1, rho_g2))
    rho2s.append(rho_g2)
    weighted_avg = 0
    for n in range(7):
        weighted_avg += rho_g1[n]*n
        weighted_avg += rho_g2[n]*n
    weighted_avg = weighted_avg/12
...
plt.legend()
plt.savefig("n_fracs_halfway.pdf")
plt.close()